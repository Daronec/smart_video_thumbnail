  ...
  group('Cache Properties', () {
    ...
    test('Property 1: Cache round-trip consistency', () async {
      ...
      for (int i = 0; i < iterations; i++) {
        ...
        final strategy = ['normal', 'keyframe', 'firstFrame'][random.nextInt(3)];
        ...
        // Generate cache key
        final cacheKey = cache.generateKey(videoPath, timeMs, width, height, strategy);
        ...
        expect(retrievedData!.length, equals(originalData.length),
          reason: 'Iteration $i: Retrieved data length should match original');
        expect(retrievedData, equals(originalData),
          reason: 'Iteration $i: Retrieved data should be identical to original');
      }
    });
    ...
    test('Property 2: Cache key determinism', () {
      ...
      for (int i = 0; i < iterations; i++) {
        ...
        final key1 = cache.generateKey(videoPath, timeMs, width, height, strategy);
        final key2 = cache.generateKey(videoPath, timeMs, width, height, strategy);
        final key3 = cache.generateKey(videoPath, timeMs, width, height, strategy);
        
        // Verify determinism
        expect(key1, equals(key2),
          reason: 'Iteration $i: Keys should be identical for same parameters');
        expect(key2, equals(key3),
          reason: 'Iteration $i: Keys should be identical across multiple calls');
    ...
    test('Property 3: Cache key uniqueness', () {
      ...
      for (int i = 0; i < iterations; i++) {
        ...
        final key = cache.generateKey(videoPath, timeMs, width, height, strategy);
        
        // Verify uniqueness
        expect(seenKeys.contains(key), isFalse,
          reason: 'Iteration $i: Key should be unique for different parameters');
    ...
    test('Property 5: Cache key collision resistance', () {
      ...
      final baseKey = cache.generateKey(basePath, baseTime, baseWidth, baseHeight, baseStrategy);
      
      // Test variations
      final variations = [
        cache.generateKey('$basePath.', baseTime, baseWidth, baseHeight, baseStrategy),
        cache.generateKey(basePath, baseTime + 1, baseWidth, baseHeight, baseStrategy),
        cache.generateKey(basePath, baseTime, baseWidth + 1, baseHeight, baseStrategy),
        cache.generateKey(basePath, baseTime, baseWidth, baseHeight + 1, baseStrategy),
        cache.generateKey(basePath, baseTime, baseWidth, baseHeight, 'keyframe'),
      ];
